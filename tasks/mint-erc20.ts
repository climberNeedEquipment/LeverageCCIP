...
import { DestinationMinter, IERC20, MyNFT__factory } from "../typechain-types";
import { Spinner } from "../utils/spinner";

task(`mint-erc20`, `Mints several ERC20 tokens to EOA`)
  .addOptionalParam(
    `router`,
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      console.log(
        `ℹ️  Attempting to mint MyNFT smart contract on the ${hre.network.name} blockchain using ${deployer.address} address`
      );
      spinner.start();

      const ERC20: IERC20 = await hre.ethers.deployContract("ERC20");
